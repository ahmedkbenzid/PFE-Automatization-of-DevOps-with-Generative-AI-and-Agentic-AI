...
class RAGKnowledgeBase:
    ...
    def query(self, query_text: str, top_k: int = 3, use_enhanced: Optional[bool] = None) -> List[Dict[str, Any]]:
        ...
        if should_use_enhanced:
            results = self._query_with_enhanced_chunking(query_text, pages, top_k * 3)
        else:
    ...
    def _post_process_results(self, query_text: str, results: List[Dict[str, Any]], top_k: int) -> List[Dict[str, Any]]:
        ...
        for idx, page in enumerate(results):
            ...
            # Strong stack matching: boost matching stacks, penalize mismatches
            if expected_stack:
                if self._is_stack_match(searchable, expected_stack):
                    score += 50.0  # Large boost for matching stack
                    print(f"[Docker RAG] MATCH: {page_id} matches {expected_stack} (+50.0)")
                else:
                    # Penalize if it's clearly a different stack
                    for other_stack in ["node", "python", "go", "rust", "java"]:
                        if other_stack != expected_stack and self._is_stack_match(searchable, other_stack):
                            score -= 30.0  # Large penalty for wrong stack
                            print(f"[Docker RAG] MISMATCH: {page_id} is {other_stack}, not {expected_stack} (-30.0)")
                            break
        ...
        for score, page in scored[:top_k]:
            print(f"  - {page.get('page_id', 'unknown')}: {score}")
