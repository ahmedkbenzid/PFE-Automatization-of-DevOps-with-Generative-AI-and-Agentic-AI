...
class ExecutionPipeline:
    ...
    def _write_act_secret_file(self, runtime_secrets: Optional[Dict[str, str]] = None) -> Optional[str]:
        ...
        The file is created with owner-read-only permissions (0o600) so other local
        users cannot read it via /proc or ps.  The caller is responsible for deleting
        the file in a finally block.
        Returns the path to the secret file, or None if there are no secrets.
        """
        secrets = self._collect_act_secrets(runtime_secrets)
        if not secrets:
            return None

        fd, secret_file_path = tempfile.mkstemp(prefix="act-secrets-", suffix=".env")
        try:
            # Restrict permissions before writing any sensitive content.
            os.chmod(secret_file_path, stat.S_IRUSR | stat.S_IWUSR)
            with os.fdopen(fd, "w", encoding="utf-8") as fh:
                for key, value in secrets.items():
                    # Escape newlines inside values to keep one KEY=VALUE per line.
                    safe_value = value.replace("\n", "\\n")
                    fh.write(f"{key}={safe_value}\n")
        except Exception:
            # Clean up on write failure so we don't leave a partial secrets file.
            try:
    ...
    def _build_act_secret_args(self, secret_file_path: Optional[str]) -> List[str]:
        """
        FIX (security): Return --secret-file args referencing the temp file instead
        of expanding every KEY=VALUE on the command line.
        """
        if not secret_file_path:
            return []
        return ["--secret-file", secret_file_path]
    ...
    # into one method that accepts a plain list of log dicts and uses the
    # full combined set of markers.
    # ------------------------------------------------------------------
    def _is_transient_network_error_from_logs(self, logs: List[Dict[str, Any]]) -> bool:
        """Detect transient network/TLS failures from a list of log entry dicts."""
        network_markers = (
    ...
    def _is_transient_network_error(self, act_result: Dict[str, Any]) -> bool:
        """Detect transient network/TLS failures from an act result dict."""
        logs = act_result.get("logs", []) if isinstance(act_result, dict) else []
        if not isinstance(logs, list):
            return False
        return self._is_transient_network_error_from_logs(logs)
    ...
    def _resolve_workflow_file(self, workspace_path: Path) -> str:
        ...
        preferred_names = [
            "ci.yml", "ci.yaml", "main.yml", "main.yaml",
    ...
    def _run_act_with_fallback_images(
        ...
        # FIX (security): write secrets to a temp file; delete it in finally.
        secret_file_path: Optional[str] = None
        try:
            secret_file_path = self._write_act_secret_file(runtime_secrets)
            secret_args = self._build_act_secret_args(secret_file_path)
            ...
            for image in fallback_images:
                fallback_cmd = [
                    "act", "-W", workflow_file,
                    *common_args, *secret_args, *extra_args,
            ...
            if secret_file_path:
                try:
                    ...
                    self.logger.warning(f"Could not delete secret file {secret_file_path}: {exc}")
    ...
    def execute(
        ...
        Args:
            ...
            docker_timeout: Timeout in seconds for Docker build / image-inspect steps
            act_timeout: Timeout in seconds for Act execution (default: 1800 = 30 minutes)
            secrets: Optional dict of secret key/value pairs passed to act
            prebuilt_image_name: Existing local Docker image tag/name to use instead of building
            log_callback: Optional callable invoked for each log line in real time

        Returns:
            Dictionary with execution results including:
            - status: "success" or "error"
            - message: Summary message
            - workspace: Path to temporary workspace (already cleaned up on return)
            - docker_build: Docker build result (or skipped sentinel)
        ...
        # FIX (critical): use TemporaryDirectory so the workspace is always
        # cleaned up — even on early-return error paths or uncaught exceptions.
        # FIX (minor): built Docker image is removed in the same finally block.
        temp_dir_ctx = tempfile.TemporaryDirectory(prefix="exec-agent-")
        workspace_path = Path(temp_dir_ctx.name)
        self.logger.info(f"Created temporary workspace: {workspace_path}")

        # Track image_tag so we can remove it in cleanup.
        ...
        try:
            ...
            # FIX (design): derive the workflow filename via _resolve_workflow_file
            # so the write path always matches what act will look for.
            ...
            image_name = f"execution-generated-{uuid.uuid4().hex[:12]}"
            ...
            # FIX (minor): remove the built image from the Docker daemon.
            if built_image_tag:
                try:
                    subprocess.run(
                        ["docker", "rmi", "--force", built_image_tag],
                        capture_output=True,
                        check=False,
                    )
                    self.logger.info(f"Removed Docker image: {built_image_tag}")
                except Exception as exc:
                    self.logger.warning(f"Could not remove Docker image {built_image_tag}: {exc}")

            # FIX (critical): always clean up the temporary workspace.
            try:
                temp_dir_ctx.cleanup()
            except Exception as exc:
                self.logger.warning(f"Could not clean up workspace {workspace_path}: {exc}")
    ...
    def _run_command_with_timeout(
        ...
        Returns a dict with: step, command, cwd, exit_code, timed_out, logs, success.
        """
        ...
        while True:
            ...
            if elapsed > timeout_seconds:
                ...
                timed_out = True
                process.kill()
                # FIX (warning): join reader threads after kill so we drain
                # any remaining buffered output and avoid resource leaks.
            ...
            try:
                ...
                # FIX (warning): add a sentinel when the cap is reached so
                # callers know the log was truncated rather than silently losing lines.
                if len(collected_logs) < 2000:
                    ...
                    collected_logs.append({
                        "stream": "stderr",
                        "line": "[pipeline.py] Log truncated: output exceeded 2000 lines. Subsequent lines omitted.",
                    })
            except queue.Empty:
                pass
            ...
            try:
                ...
                if log_callback:
                    try:
                        ...
                        self.logger.error(f"log_callback error: {e}")
                if len(collected_logs) < 2000 and not log_truncated:
                    collected_logs.append(log_entry)
            except queue.Empty:
                break
...
def run_execution(
    ...
    Args:
        dockerfile_content: Dockerfile content (used when no prebuilt image is provided)
        cicd_workflow_content: Generated CI/CD workflow content
        repository_path: Path to source repository
        github_url: Optional GitHub URL to clone from
        docker_timeout: Timeout in seconds for Docker build / image-inspect steps
